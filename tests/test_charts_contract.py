...
def test_r20_1_histogram_example_uses_common_plotspec():
...
def test_r20_1_version_bumped():
    root=Path(__file__).resolve().parents[1]; assert 'version = "0.2.0a17"' in (root/"pyproject.toml").read_text(encoding="utf-8"); assert '__version__ = "0.2.0a17"' in (root/"sumgui"/"__init__.py").read_text(encoding="utf-8");
