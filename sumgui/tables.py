...
class TableView(Widget):
    ...
    def draw(self, screen):
        ...
        top = self.rect.y + 4;
        if self.spec.title:
            title_rect = pygame.Rect(self.rect.x + 6, top, self.rect.width - 12, self.font.get_height());
            draw_clipped_text(screen, self.font, self.spec.title, self.theme.title, title_rect);
            top += self.font.get_height() + 4;
        rows = list(self.spec.rows);
        headers = list(self.spec.headers);
        column_count = max([len(headers)] + [len(row) for row in rows] + [1]);
        row_height = max(self.font.get_height() + 6, 18);
        ...
        def draw_inside():
            ...
            if headers:
                for col in range(column_count):
                    ...
                    draw_clipped_text(screen, self.font, str(text), self.theme.button_text, cell.inflate(-6, -2), valign="middle");
                y += row_height;
